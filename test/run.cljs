...
;;   runner that destructures `{:keys [fail error]}` from it computes
;;   `(+ nil nil)` = 0 and calls `(js/process.exit 0)` over a real failure,
;;   clobbering the nonzero code nbb had already put on process.exitCode.
;;   Measured: a deliberately broken assertion printed "FAIL in (...)" and
;;   "1 failures, 0 errors." and the runner still exited 0.
;;
;; `:end-run-tests` on the default reporter is the hook that fires once the
;; whole run block has drained — synchronous and async alike — and carries the
;; real summary map. We `set!` process.exitCode rather than calling
;; process.exit, so Node still flushes stdout and finishes pending work.
(require '[clojure.test :as t] 'okugai.medium-test 'okugai.facts-test 'okugai.site-test 'okugai.operator-test)
...
(t/run-tests 'okugai.medium-test 'okugai.facts-test 'okugai.site-test 'okugai.operator-test)
