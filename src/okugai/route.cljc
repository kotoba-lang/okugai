...
(def statuses #{:routable :operator-known :unresolved})
...
(defn stamp
  ...
  **上書きしない**（媒体固有の解決の方が強い）。"
  [site]
  (if (:site/route-status site)
    site
    (assoc site :site/route-status
           (if (blank? (:site/operator-raw site)) :unresolved :operator-known))))
...
(defn reachable?
  ...
  [site]
  (contains? #{:routable :operator-known :candidate-by-area} (:site/route-status site)))
